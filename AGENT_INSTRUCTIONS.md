...
1. **Owner VIP Exemption & Real Paid Features Enforcement (`AuthContext.tsx`, `plus/page.tsx`, `TodoList.tsx`)** (Checkpoint 18):
...
2. **Obour Hub VIP Pass / Subscription System (`/plus`)** (Checkpoint 17):
...
3. **Leaderboard & Season Ceremony Integration (`/community`)** (Checkpoint 16):
...
4. **Cup Emoji Clean Removal (`Sidebar.tsx`, `/community`)** (Checkpoint 16):
...
5. **Mandatory Onboarding Registration Enforcement for Existing Users (`AppShell.tsx`, `StudentProfileSetup.tsx`)** (Checkpoint 16):
...
6. **Google Login Terms & Privacy Agreement Disclaimer (`LoginScreen.tsx`)** (Checkpoint 15):
...
7. **Study Buddies Smart Matchmaker Search & Filters (`/buddies`)** (Checkpoint 14):
...
8. **Academic Tasks Batch "Clear Completed" Action (`/todo`)** (Checkpoint 14):
   - Added batch deletion capability for finished tasks in `TodoList.tsx`.
9. **Student Project Showcase Search & Zod Validation (`/showcase`)** (Checkpoint 13):
...
10. **Alumni Network & Internship Search Board (`/alumni`)** (Checkpoint 13):
...
11. **Student Guide & Interactive FAQ Accordion (`/guide`)** (Checkpoint 13):
...
12. **Student Gear Marketplace (`/market`)** (Checkpoint 13):
...
16. **Academic Schedule & Day Filter Pills (`/schedule`)** (Checkpoint 12):
...
17. **Interactive MindMap Visualizer (`/mindmap`)** (Checkpoint 12):
    - Upgraded input form and concept tree renderer to solid cards (`bg-card border border-border dark:bg-card`).
18. **Kanban vs List View Toggle on Tasks Page (`/todo`)** (Checkpoint 11):
...
19. **Resource Bookmarking & Tagging on Subject Details (`/subject`)** (Checkpoint 11):
...
20. **Optimistic Upvoting & Subject Tags on Q&A Forum (`/qa`)** (Checkpoint 11):
...
21. **Past Exams Multi-Filtering & Solution Key Preview Drawer (`/exams`)** (Checkpoint 11):
...
22. **Code Cleanup, Security & Dynamic Imports** (Checkpoint 11 & 12):
