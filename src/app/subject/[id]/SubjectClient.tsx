...
import { ArrowLeft, Download, ExternalLink, FileText, Link as LinkIcon, Loader2 } from 'lucide-react';
          ...
          {resources.length === 0 ? (
            <div className="text-center py-16 bg-muted/30 rounded-2xl border-2 border-dashed border-border">
              ...
              <p className="text-muted-foreground">
                {language === 'ar' ? 'لا توجد موارد حالياً' : 'No resources available yet'}
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {resources.map((resource) => (
                <div key={resource.id} className="group bg-card p-4 rounded-xl border border-border hover:shadow-lg transition-all flex items-center justify-between">
