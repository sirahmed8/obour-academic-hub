...
describe("PUT /api/admin/users/[uid]/ban", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  ...
  it("bans the user successfully after verifying permissions", async () => {
    const { PUT } = await import("./route");

    const response = await PUT(
      new Request("http://localhost/api/admin/users/student-1/ban", {
        method: "PUT",
        headers: {
    ...
    expect(updateUserMock).toHaveBeenCalledWith("student-1", { disabled: true });
