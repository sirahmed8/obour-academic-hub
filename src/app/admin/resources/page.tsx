...
export default function AdminResourcesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm(prev => ({ ...prev, title: '', description: '', linkUrl: '', file: null }));
    } catch (error) {
      ...
      <div className="p-6 lg:p-10 max-w-2xl mx-auto">
        ...
        <div className="bg-card p-6 rounded-2xl border border-border">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-4">
              <button
                ...
                className={cn(
                  ...
                  form.type === 'pdf' ? 'border-primary bg-primary/10 text-primary' : 'border-border opacity-60 hover:opacity-100'
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "flex-1 p-4 rounded-xl border-2 flex flex-col items-center gap-2 transition-all",
                  form.type === 'link' ? 'border-purple-500 bg-purple-50 text-purple-600' : 'border-border opacity-60 hover:opacity-100'
                )}
              >
                <LinkIcon className="w-6 h-6" />
                <span className="font-bold">{language === 'ar' ? 'رابط' : 'Link'}</span>
              </button>
            </div>
            ...
            {form.type === 'link' ? (
              <div>
                <label className="block text-sm font-medium mb-1">{language === 'ar' ? 'الرابط' : 'URL'}</label>
                <input 
                  type="url" 
                  value={form.linkUrl}
              ...
              <div>
                ...
                <div className="border-2 border-dashed border-border rounded-xl p-6 text-center hover:bg-muted/30 transition-colors cursor-pointer relative">
                  ...
                  <div className="flex flex-col items-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-sm">{form.file ? form.file.name : (language === 'ar' ? 'اضغط لاختيار ملف PDF' : 'Click to select PDF')}</span>
                  </div>
                </div>
              </div>
            )}
