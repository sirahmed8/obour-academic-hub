...
export function DateTimePicker({ value, onChange, onClose, language }: DateTimePickerProps) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mt-2 z-50 rounded-2xl shadow-xl overflow-hidden outline-none will-change-transform bg-popover/80 backdrop-blur-2xl border border-border/50 min-w-[300px]"
      )}
      dir={isRtl ? "rtl" : "ltr"}
            ...
            <div key={index} className="aspect-square">
              {day && (
                <motion.button
                  ...
                  {...getHoverProps(shouldReduceMotion)}
                  className={cn(
                    "w-full h-full rounded-xl text-sm font-medium transition-all flex items-center justify-center",
                    isSelected(day)
