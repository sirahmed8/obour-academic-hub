...
export function AdminApprovalModal() {
  ...
  const handleAction = async (approved: boolean) => {
    ...
    try {
      if (approved) {
        await updateDoc(doc(db, "admin_approvals", currentRequest.id), {
          status: "approved",
          approvedBy: user?.uid,
      ...
      <div className="fixed inset-0 z-100 flex items-end sm:items-center justify-center pointer-events-none p-4 pb-8 sm:pb-4">
        ...
        */}
        ...
        <motion.div
          ...
          className="pointer-events-auto w-full max-w-sm sm:max-w-md bg-white/80 dark:bg-zinc-900/80 backdrop-blur-xl border border-white/20 shadow-2xl rounded-3xl overflow-hidden ring-1 ring-black/5"
        >
          {/* Header */}
          ...
          <div className="p-4 bg-muted/30 flex gap-2">
            <button
              ...
              className="flex-1 py-2.5 rounded-xl border border-red-200 bg-red-50 hover:bg-red-100 text-red-600 font-medium text-sm transition-colors flex items-center justify-center gap-2"
            >
              <X size={18} />
            ...
            <button
              ...
              className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 transition-colors flex items-center justify-center gap-2 shadow-sm"
            >
              <Check size={18} />
