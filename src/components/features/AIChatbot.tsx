...
export function AIChatbot() {
  ...
  // Memoized handlers to prevent re-renders of ChatMessageItem
  const handleReplyClick = useCallback((msg: ChatMessage) => {
    setReplyTo(msg);
  }, []);
  ...
  const handleReactionClick = useCallback(
    (msg: ChatMessage, emoji: string) => {
      if (!user) return;
      toggleReaction(user.uid, msg.id, user.uid, emoji);
    },
    [user]
  );
