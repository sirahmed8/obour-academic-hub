...
import { TodoTask, Subject } from "@/types";
...
export function AddTodoModal({
  ...
  useEffect(() => {
    ...
    const unsubscribe = subjectService.getSubjects((data) => {
      setSubjects(data);
    });
    return () => unsubscribe();
  }, [isOpen, user]);
  ...
  const handleAISubtaskBreakdown = async () => {
    if (!title.trim() || isGeneratingSubtasks) return;
      ...
      const data = await apiFetch<{ content: string }>("/api/ai/task-planner", {
        ...
        body: {
          messages: [
            {
              role: "user",
              content: `قم بتقسيم المهمة التالية إلى 3-4 خطوات تفصيلية عمليّة: "${title}". ${description ? `التفاصيل: ${description}` : ""}`,
            },
          ],
        },
      });

      const match = data.content?.match(/\[TASK_SPEC:\s*({[\s\S]*?})\]/);
      if (match && match[1]) {
        const spec = JSON.parse(match[1]);
        if (spec.subtasks && Array.isArray(spec.subtasks)) {
          const generated = spec.subtasks.map((st: string) => ({
            id: crypto.randomUUID(),
            title: st,
            completed: false,
          }));
          setSubtasks((prev) => [...prev, ...generated]);
          toast.success(isRtl ? "تم تقسيم المهمة بالذكاء الاصطناعي!" : "Subtasks generated by AI!");
        }
      } else {
        toast.info(isRtl ? "تمت إضافة اقتراح المهمة" : "Subtask breakdown updated");
      }
    } catch {
      toast.error(isRtl ? "فشل في توليد الخطوات" : "Failed to generate subtasks");
    } finally {
        ...
        >
          ...
          >
            ...
            <div ref={scrollContainerRef} className="p-5 space-y-5">
              ...
              {/* Date Picker - Full width outside grid */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-1 bg-background/50 p-1.5 rounded-xl border border-border h-[46px]">
                    {repeats.map((r) => (
                      <motion.button
                        key={r.value}
                        ...
                        whileTap={{ scale: 0.95 }}
                        className={cn(
                          "flex-1 text-[10px] sm:text-xs py-1 px-1 rounded-lg font-medium transition-all border border-transparent flex items-center justify-center",
                          repeat === r.value
                ...
                <div className="space-y-1.5 relative z-40">
                  ...
                  <CustomSelect
                    options={[
                      { value: "", label: language === "ar" ? "بدون مادة" : "No Subject" },
