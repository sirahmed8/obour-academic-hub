      ...
      <motion.div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
        <HoverScale scale={1.02}>
          <TapScale scale={0.97}>
            <div
              className={cn(
                "group relative overflow-hidden flex flex-col justify-between h-full p-6 sm:p-7 rounded-[2rem] focus-visible:ring-2 focus-visible:ring-primary border border-border bg-card shadow-md hover:shadow-2xl transition-all duration-300 dark:bg-card cursor-default"
              )}
            >
              ...
              <div>
                <div className="flex items-start justify-between gap-3 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {(() => {
                      const sObj = subject as unknown as {
                        code?: string;
                        level?: number;
                        year?: number;
                      };
              ...
              <div className="mt-6 pt-4 border-t border-border/40 space-y-3">
                ...
                {user && (
                  <div className="flex items-center gap-1.5 text-[11px] font-bold text-emerald-500 dark:text-emerald-400">
                    ...
                    <span>
                      {language === "ar" ? "مسجل في خطتك الدراسية" : "Enrolled in Pathway"}
                    </span>
                  </div>
                )}
              </div>
